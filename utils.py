...
def load_images_and_flows(dataset, new_size=[128, 192], train=True, force_recalc=False):
    ...
    for i in range(n_clip):
        ...
        if os.path.isfile(saved_image_file) and not force_recalc:
            ...
            image_data = np.array([extend_gray_channel(resize(imageio.v2.imread(img_file)/255., (new_size[1], new_size[0])))
...
def basic_assess_AUC(scores, labels, plot_pr_idx=None):
    ...
    if plot_pr_idx is not None:
        ...
        plt.ylim([0.0, 1.05])
        plt.show()
    return np.array([roc_auc_score(labels, scores[:, i]) for i in range(scores.shape[1])]), np.array([average_precision_score(labels, scores[:, i]) for i in range(scores.shape[1])])
...
def assess_Boat_Belleview_Train(dataset, epoch, plot_pr=True):
    def load_grid_ground_truth_Boat(path_test):
        ...
        print(len(filenames))
        data = np.array([imageio.v2.imread(filename, 'L')/255. for filename in filenames])
