...
def train_Unet_naive_with_batch_norm(training_es_images, training_ed_images, max_epoch, dataset_name='', start_model_idx=0, batch_size=16,
    ...
    G_loss_total = lw_adv*G_loss + lw_appe*loss_appe + lw_aux*loss_ed
    ...
    with tf.Session(config=config) as sess:
        ...
        _val_disease_ed   = {}   # disease_label -> [mean_ed   per epoch]
        ...
        for i in range(start_model_idx, max_epoch):
            ...
            wandb.log({
                ...
                "charts/ED_Prediction_Loss_Components": _wandb_line_series(
                    xs=_steps,
            ...
            if val_es_images is not None and val_ed_images is not None and len(val_es_images) > 0:
                ...
                if val_labels is not None and len(val_labels) == len(val_es_images):
                    ...
                    for disease in sorted(np.unique(labels_arr)):
                        ...
                        d_ed   = float(np.mean(per_sample_ed[mask]))
                        print('  [VAL-%s] appe = %.4f, ed = %.4f  (%d samples)'
                              % (disease, d_appe, d_ed, np.sum(mask)))
                        if disease not in _val_disease_appe:
                            _val_disease_appe[disease] = []
                            _val_disease_ed[disease]   = []
                        _val_disease_appe[disease].append(d_appe)
                        _val_disease_ed[disease].append(d_ed)
                        ...
                        for _disease in sorted(np.unique(labels_arr)):
                            ...
                            if np.any(_bin_d) and not np.all(_bin_d):
                                try:
                                    ...
                                    _auc_d_aux  = roc_auc_score(_bin_d, per_sample_ed[_mask_d])
                                    _per_disease_aucs[_disease] = (_auc_d_appe, _auc_d_aux)
                                    print('  [VAL-AUC-%s] appe = %.4f, ed = %.4f'
                ...
                if not np.isnan(val_healthy_appe) and not np.isnan(val_unhealthy_appe):
                    ...
                    log_dict["charts/Val_ED_Prediction_Loss_by_Group"] = _wandb_line_series(
                        xs=_val_epochs,
                    ...
                    if _val_disease_appe:
                        ...
                        log_dict["charts/Val_ED_Prediction_Loss_by_Disease"] = _wandb_line_series(
                            xs=_val_epochs,
                            ys=[_val_disease_ed[d] for d in sorted_diseases],
                            keys=sorted_diseases,
                            title="Val ED Prediction Loss per Disease",
                            xname="Global Step"
                        )
